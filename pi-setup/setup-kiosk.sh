...
LAUNCH="$HOME/.local/bin/flight-tracker-kiosk.sh"
mkdir -p "$HOME/.local/bin"
cat > "$LAUNCH" <<EOF
#!/usr/bin/env bash
# Kiosk launcher. Chromium's native-Wayland GPU path can be flaky on the Pi, so
# we run it through Xwayland (--ozone-platform=x11) for a stable kiosk.
export DISPLAY=:0
...
if command -v labwc >/dev/null 2>&1; then
  ...
  grep -q flight-tracker-kiosk "$AUTOSTART" 2>/dev/null || echo "$LAUNCH &" >> "$AUTOSTART"
  ...
  grep -q flight-tracker-kiosk "$INI" || sed -i "/\[autostart\]/a flighttracker = $LAUNCH" "$INI"
