  ...
  if (!cfg) {
    return (
      <div className="loading">
        ...
        {state.connected ? "Loading config…" : "Connecting to flight tracker…"}
      </div>
    );
  }
    ...
    <div className="control">
      ...
      <main>
        ...
        <Section title="Orientation">
          <Row label="Rotation" hint="align map to the screen">
            <Slider value={cfg.rotationDeg} min={0} max={355} step={5} unit="°"
              onChange={(v) => set({ rotationDeg: v })} />
          </Row>
          <Row label="Mirror horizontally" hint="for awkward mounts">
