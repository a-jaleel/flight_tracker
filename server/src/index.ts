...
const GEOCODE_UA =
  ...
  "flight_tracker/0.1 (https://github.com/a-jaleel/flight_tracker)";
const CONFIG_PATH = resolve(DATA_DIR, "config.json");

async function main(): Promise<void> {
  const store = new ConfigStore(CONFIG_PATH, DEFAULT_CONFIG);
  await store.load();
  ...
  server.listen(PORT, HOST, () => {
    ...
    console.log(`[server] data source: airplanes.live API (${API_URL})`);
